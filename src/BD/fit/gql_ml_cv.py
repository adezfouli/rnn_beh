# This runs cross-validation for GQL (d=2). Note that the output of this file is only the saved models (and not the
# actual performance of the model on the test data). For evaluating how the saved models perform
# on test dataset, file 'sim/gql_ml_cv.py' should be
...
def run_BD_Q(i):
    ...
    output_path = Paths.local_path + 'BD/gql-ml-cv/' + group + '/' + 'fold' + str(cv_index) + '/'
