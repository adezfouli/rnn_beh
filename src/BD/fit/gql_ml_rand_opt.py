# This file optimises GQL (d=2), but fits a separate model to each subject (instead of fitting a single model to
# the whole group).
...
for i in range(len(cv_lists_group['Healthy'])):
    configs.append({'g': 'Healthy', 'lr': 1e-1, 'cv_index': i, 'iters': 500})


for i in range(len(cv_lists_group['Depression'])):
    configs.append({'g': 'Depression', 'lr': 1e-1, 'cv_index': i, 'iters': 500})


for i in range(len(cv_lists_group['Bipolar'])):
    configs.append({'g': 'Bipolar', 'lr': 1e-1, 'cv_index': i, 'iters': 500})
...
def run_BD_GQL(i):
    ...
    output_path = Paths.local_path + 'BD/gql-ml-rand-opt/' + group + '/' + 'fold' + str(cv_index) + '/'
